...
function MediaCard({ item }: { item: VideoListItem }) {
  ...
  return (
    <Link href={`/user/media/${item.video_id}`} className="group block focus:outline-none focus:ring-2 focus:ring-primary/20">
...
export default function MediaPage() {
  ...
  const params = useMemo(
    () => ({
      ...
      type: type || undefined,
      with_media: withMedia ? withMedia === "true" : undefined
    }),
    ...
    <div className="space-y-4 lg:space-y-5">
      <GlassPanel className="p-5 sm:p-6 lg:p-8">
        ...
        <div className="mt-3 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <h1 className="text-2xl font-semibold tracking-tight sm:text-3xl">媒体库</h1>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">浏览 EMOS 视频目录，支持标题搜索、类型筛选、资源状态筛选和下拉加载更多。</p>
          </div>
          <div className="rounded-2xl border border-border/50 bg-muted/15 px-4 py-3 text-sm text-muted-foreground">
            已加载 <span className="font-mono text-foreground">{items.length}</span> / <span className="font-mono text-foreground">{total}</span>
          </div>
        </div>
      </GlassPanel>
