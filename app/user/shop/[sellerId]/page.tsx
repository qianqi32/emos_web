...
import { ArrowLeft, Copy, Loader2, Package, RefreshCw, Search, Store } from "lucide-react";
import Link from "next/link";
import { use, useCallback, useEffect, useMemo, useRef, useState } from "react";
import { GlassPanel } from "@/components/ui/glass-panel";
import { getShopCategoryList, getShopProductList, getShopSellerBase } from "@/lib/api/client";
import type { ShopCategory, ShopProductItem, ShopSellerBase } from "@/lib/api/types";
        ...
        <>
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5">
            {products.map((product) => (
              <GlassPanel key={product.product_id} className="overflow-hidden p-0">
                <div className="aspect-[16/9] bg-muted/30">
                  {product.cover_url ? <div role="img" aria-label={product.name} className="h-full w-full bg-cover bg-center" style={{ backgroundImage: `url(${product.cover_url})` }} /> : <div className="flex h-full w-full items-center justify-center"><Package className="h-8 w-8 text-muted-foreground" /></div>}
                </div>
                <div className="p-4">
                  <div className="line-clamp-1 text-sm font-semibold">{product.name}</div>
                  <div className="mt-2 text-xs text-muted-foreground">库存 {product.stock} · 销量 {product.sales}</div>
